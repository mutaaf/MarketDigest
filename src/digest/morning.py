...
def build_morning_digest(builder: DigestBuilder, mode: str = "facts", out_data: dict | None = None) -> str:
    ...
    ALL_SECTIONS = ["quick_take", "overnight", "futures", "forex", "commodities",
                    "crypto", "events", "sentiment", "next_steps"]
    enabled = get_enabled_sections("morning", ALL_SECTIONS)
    ...
    # Always compute sentiment + save morning snapshot (afternoon depends on it)
    try:
    ...
    try:
        custom_sources = builder.fetch_custom_sources("morning")
